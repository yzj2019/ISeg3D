...
class Agile3dSegmentor(nn.Module):
    def __init__(self, pcd_backbone=None, mask_decoder=None, loss=None, 
                 semantic=True, clicks_from_instance=False, 
                 max_train_iter=3, mask_threshold=0.):
        '''
    ...
    def refine(self, pred, pcd_dict, clicks):
        ...
        if len(clicks) == 0:
            return pred
        masks_heatmap, cls_logits = self.mask_decoder(pcd_dict, clicks, pred)      # N_points x N_clicks, N_clicks x num_classes
    ...
    def forward_train(self, pcd_dict, clicker, img_dict, fusion):
        ...
        pcd_dict["feature_maps"] = backbone_res_dict["feature_maps"]    # pcd embedding list, [(N_points, embedding_dims[i])]

        clicks = clicker.make_init_clicks()     # 获取 clicks simulation
        masks_pred, cls_logits, pred_refine = self.refine(pcd_pred, pcd_dict, clicks)    # 优化pred
        clicks_dict = dict(
            masks_pred=masks_pred, cls_logits=cls_logits,
        ...
        return dict(loss=loss, clicks_mask_loss=info_dict["clicks_mask_loss"], clicks_cls_loss=info_dict["clicks_cls_loss"], 
    ...
    def forward(self, input_dict):
        ...
        # test
            ...
            clicker.set_state('test', [pcd_dict],
                              semantic=input_dict["semantic"], instance=input_dict["instance"],
                              img_dict=img_dict, fusion=fusion)
