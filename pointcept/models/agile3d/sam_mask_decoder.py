...
from .misc import LayerNorm2d, _get_activation_fn, MLPBlock
...
class Attention(nn.Module):
    """
    最普通的MHA, 默认 batch_first=True, 没有dropout、layernorm啥的, 把positional embedding写在外面了, 可选downsample
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        ...
        attn = q @ k.permute(0, 1, 3, 2)  # 乘完后 B x N_heads x N_tokens x N_tokens
...
# --------------------------------------------------------------------------------------------------
# 最普通的 attention layer, 用 nn.MultiheadAttention 实现, 没用 .attention.py
# 有些直接不定义这里的 attention layer, 直接拆开写在 transformer layer 里了, 如 TwoWayTransformerBlock
# transformer layer 通常需要 positional embedding
# --------------------------------------------------------------------------------------------------
class SelfAttentionLayer(nn.Module):
    def __init__(
        ...
        # 其实好像没用上激活函数，因为只是封装attention计算成layer，而不是transformer layer
...
# -------------------------------------------------------------------
# from Segment Anything
# 双通道 transformer block & decoder, 互相查询, 用的是自己实现的mhattn
# -------------------------------------------------------------------
class TwoWayTransformerBlock(nn.Module):
    def __init__(
        ...
        Arguments:
          ...
          dropout_list: dropout percent in each layer (unused)
        """
        super().__init__()
...
class TwoWayTransformerDecoder(nn.Module):
    def __init__(
        self,
        depth: int,
        embedding_dim: int,
