...
class Agile3dDecoderBlock(nn.Module):
    ...
    def forward(
        ...
        clicks = self.click_to_click(x=clicks, x_batch=clicks_batch, pe=None if self.skip_first_layer_pe else clicks_pe)

        clicks = self.mlp(clicks)

        scene = self.scene_to_click(source=clicks, query=scene,
                            source_batch=clicks_batch, query_batch=scene_batch, 
                            attn_mask=attn_mask, 
...
class Agile3dMaskDecoder(nn.Module):
    def __init__(
        ...
        assert len(with_attn_mask) == block_num + int(enable_final_block), f"len(with_attn_mask) must be same as {block_num + int(enable_final_block)}, but got{len(with_attn_mask)}"
    ...
    def transformer_fwd(self, clicks_embedding, clicks_pe, clicks_batch, scene_embedding, scene_pe, scene_batch, last_masks_heatmap=None):
        '''
        transformer forward
        ...
        for i in range(self.depth):
            ...
            for j in range(self.block_num):
                ...
                if self.with_attn_mask[j]:
                    ...
                    assert attn_mask.shape[0] == clicks_embedding.shape[0] and attn_mask.shape[1] == scene_embedding.shape[0], f"{attn_mask.shape}, {clicks_embedding.shape[0]}, {scene_embedding.shape[0]}, {j}"
                
                scene_embedding, clicks_embedding = self.blocks[j](
                    scene=scene_embedding, clicks=clicks_embedding,
    ...
    def get_clicks_index(self, clicks_list:List[MyClick], scene_dict):
        ...
        discrete_coord = torch.cat([batch.unsqueeze(-1).int(), scene_dict["discrete_coord"].int()], dim=1).contiguous()
    ...
    def mask_module(self, masks_heatmap, N_clicks, tau=0.5):
        ...
        '''
        assert N_clicks >= masks_heatmap.shape[1], "clicks number must be no less than last clicks number, because it contains last clicks by default"
    ...
    def head(self, scene_embedding, clicks_embedding):
        ...
        # get new cls
        cls_logits = self.cls_head(clicks_embedding)        # 可以考虑head之间压缩并行? N_clicks x num_classes
        return masks_heatmap, cls_logits
    ...
    def forward(self, scene_dict, clicks_list, last_masks_heatmap):
        ...
        clicks_embedding = self.get_clicks_feat(clicks_list, scene_dict, feature_map.features)     # 从最顶层feature map生成clicks feature
        ...
        masks_heatmap, cls_logits = self.transformer_fwd(
            ...
            scene_embedding, scene_pe, scene_batch,
            last_masks_heatmap
        )
