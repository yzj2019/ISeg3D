...
class Mask3dDecoderBlock(nn.Module):
    ...
    def forward(
        ...
        clicks = self.click_to_click(
            x=clicks, pe=None if self.skip_first_layer_pe else clicks_pe
        )
...
class Mask3dMaskDecoder(nn.Module):
    def __init__(
        ...
        assert (
            len(with_attn_mask) == self.feature_maps_num
        ), f"len(with_attn_mask) must be same as feature_maps_dims {self.feature_maps_dims}, but got{len(with_attn_mask)}"
    ...
    def transformer_fwd(
        ...
        for j in range(self.feature_maps_num - 1):
            ...
            feature_map = self.in_proj_layers[j](
                feature_map.float()
            )  # (N_points_j, embedding_dim)
            ...
            attn_mask = self.downsample(
                attn_mask, scene_dict, num_pooling=self.feature_maps_num - 1 - j
            ).T  # (N_clicks, N_points_j)
            attn_mask = torch.stack(
                [attn_mask, attn_mask.clone().detach(), attn_mask.clone().detach()],
                dim=1,
            )  # (N_clicks, 3, N_points_j)
            ...
            scene_pe_down = self.downsample(
                scene_pe, scene_dict, num_pooling=self.feature_maps_num - 1 - j
            )  # (N_points, ...) -> (N_points_j, ...)
            scene_embedding, clicks_embedding = self.blocks[j](
                scene=feature_map,
                clicks=clicks_embedding,
                scene_pe=scene_pe_down,
                clicks_pe=clicks_pe,
                scene_batch=scene_batch,
                clicks_batch=clicks_batch,
                attn_mask=attn_mask,
            )
            masks_heatmap, cls_logits = self.head(
        ...
        if self.enable_final_block:
            feature_map = feature_maps[
                -1
            ].features  # (N_points, self.feature_maps_dims[j])
            feature_map = self.in_proj_layers[-1](
                feature_map
            )  # (N_points, embedding_dim)
            scene_batch = feature_maps[-1].indices[
                :, 0
            ]  # 顺序与scene_dict相同, 不需要 sort_with_indices
            attn_mask = None
    ...
    def get_scene_pe(self, scene_dict):
        ...
        for b in range(len(offset)):
            ...
            pe = self.pos_enc(
                scene_dict["grid_coord"][batch_mask].float(), input_range=[mins, maxs]
            )
            scene_pe.append(pe)
    ...
    def get_clicks_index(self, clicks, scene_dict):
        ...
        grid_coord = torch.cat(
            [batch.unsqueeze(-1).int(), scene_dict["grid_coord"].int()], dim=1
        ).contiguous()
            ...
            for class_tag in clicks[batch_id].keys():
                for name in clicks[batch_id][class_tag].keys():
                    ...
                    mask = torch.logical_and(
                        torch.logical_and(mask_x, mask_y),
                        torch.logical_and(mask_z, mask_b),
                    )
    ...
    def get_clicks_feat(self, clicks_idxs, scene_embedding):
        ...
        scene_embedding = torch.cat(
            [
                scene_embedding,
                torch.zeros((1, scene_embedding.shape[1])).to(scene_embedding.device),
            ]
        )
        return (
            scene_embedding[clicks_idxs]
            .detach()
            .clone()
            .reshape(-1, 3, scene_embedding.shape[1])
        )
    ...
    def get_clicks_pe(self, clicks_idxs, scene_pe):
        ...
        """
        scene_pe = torch.cat(
            [scene_pe, torch.zeros((1, scene_pe.shape[1])).to(scene_pe.device)]
        )
    ...
    def mask_module(self, masks_heatmap, N_clicks, tau=0.5):
        ...
        ), "clicks number must be corresponding to the last masks number"
    ...
    def head(self, scene_embedding, clicks_embedding):
        ...
        masks_embedding = self.mask_head(
            clicks_embedding
        )  # N_clicks x 3 x embedding_dim
        ...
        masks_logits = self.fus_head(masks_logits).squeeze(
            2
        )  # (N_points x N_clicks x 3) -> (N_points x N_clicks)
    ...
    def forward(self, scene_dict, clicks, last_masks_heatmap):
        ...
        """
        # 数据准备, 获取embedding
        scene_dict["feature_maps"] = scene_dict["feature_maps"][
            -self.feature_maps_num :
        ]
        feature_maps = scene_dict["feature_maps"]  # list of spconv.sparsetensor
        assert not torch.any(
            torch.isnan(feature_maps[-1].features)
        ), f"feature_maps[-1].features is nan before in_proj!"
        self.feature_used_for_masks = self.in_proj_layers[-1](
            feature_maps[-1].features
        )  # 需要梯度反传
        assert not torch.any(
            torch.isnan(self.feature_used_for_masks)
        ), f"self.feature_used_for_masks is nan after in_proj!"
        grid_coord = scene_dict["grid_coord"]
        ...
        clicks_embedding = self.get_clicks_feat(
            clicks_idxs, feature_maps[-1].features
