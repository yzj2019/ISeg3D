...
class SegmentAnything(nn.Module):
    ...
    def encode_prompt(
        ...
        if point_coords is not None:
            ...
            coords_torch = torch.as_tensor(
                point_coords, dtype=torch.float, device=self.sam.device
            )
            labels_torch = torch.as_tensor(
                point_labels, dtype=torch.int, device=self.sam.device
            )
        ...
        if mask_input is not None:
            mask_input_torch = torch.as_tensor(
                mask_input, dtype=torch.float, device=self.sam.device
            )
    ...
    def __encode_torch_prompt(
        ...
        output_tokens = torch.cat(
            [
                self.sam.mask_decoder.iou_token.weight,
                self.sam.mask_decoder.mask_tokens.weight,
            ],
            dim=0,
        )
        ...
        # Run the transformer
        hs, src = self.sam.mask_decoder.transformer(
            src, pos_src, tokens
        )  # (1, 8, 256), (1, 4096, 256)
        iou_token_out = hs[:, 0, :]
        mask_tokens_out = hs[
            :, 1 : (1 + self.sam.mask_decoder.num_mask_tokens), :
        ]  # (1, 4, 256)

        # Upscale mask embeddings, 做 ConvTranspose
        src = src.transpose(1, 2).view(b, c, h, w)  # (1, 256, 64, 64)
        upscaled_embedding = self.sam.mask_decoder.output_upscaling(
            src
        )  # (1, 32, 256, 256)
        ...
        for i in range(self.sam.mask_decoder.num_mask_tokens):
            hyper_in_list.append(
                self.sam.mask_decoder.output_hypernetworks_mlps[i](
                    mask_tokens_out[:, i, :]
                )
            )
