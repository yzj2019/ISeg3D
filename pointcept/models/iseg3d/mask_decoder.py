...
class Mask3dDecoderBlock(nn.Module):
    ...
    def forward(
        self, source: Tensor, query: Tensor, source_batch: Tensor, query_batch: Tensor,  
        source_pe=None, query_pe=None, attn_mask=None
    ) -> Tuple[Tensor, Tensor]:
        '''
        Args:
        ...
        '''
        query = self.masked_query_to_source(source=source, query=query,
                                            source_batch=source_batch, query_batch=query_batch, 
                                            attn_mask=attn_mask, 
                                            source_pe=source_pe, 
                                            query_pe=query_pe)
...
class Mask3dMaskDecoder(PointModule):
    def __init__(
        ...
        assert len(with_attn_mask) == self.features_num, f"len(with_attn_mask) must be same as feature_maps_dims {self.features_num}, but got{len(with_attn_mask)}"
        ...
        for i in range(self.features_num):
            ...
            self.blocks.append(
                nn.Identity() if i == self.features_num-1 and not enable_final_block 
    ...
    def mask_module(self, scene_feat, query_feat, scene, j, tau=0.5):
        ...
        with torch.no_grad():
            attn_mask_down = self.downsample(
                attn_mask, scene, from_resolution=self.features_num-1, to_resolution=j+1
            ).T     # (N_point[-1], N_query) -> (N_query, N_point[j+1])
            # 如果 attn_mask_down 全为 True, 则不 mask 掉, 确保一个 query 至少能查到一个点
            attn_mask_down[torch.where(attn_mask_down.sum(-1) == attn_mask_down.shape[-1])] = False
    ...
    def mask_head(self, scene_feat, query_feat, scene):
        ...
        masks_logits = scene_feat @ mask_feat.T         # embedding 相乘得到 N_query 个 mask, N_point x N_query
        masks_heatmap = masks_logits.sigmoid()          # element_wise 归一化
        # sdf refinement
        point_index, query_index = (masks_heatmap > 0.5).nonzero().T    # all instance point num N_all
        ...
        mask_query = masks_heatmap @ scene_feat.T           # N_query x embedding_dim
        mask_query = self.norm_before_head(mask_query)
        self.align_proto[query_index] = self.proto_head(torch.cat([self.align_proto[query_index], mask_query], dim=-1))
    ...
    def cls_head(self, query_feat):
        ...
        cls_logits = cls_feat @ self.sem_proto.weight.T  # N_query x num_classes, 1x1 conv
        return cls_logits
    ...
    def transformer_fwd(self, query: Query, scene: Scene, last_masks_heatmap=None):
        ...
        '''
        masks_heatmap = last_masks_heatmap.clone().detach() if last_masks_heatmap is not None else None
        ...
        for j in range(self.features_num - self.enable_final_block):
            ...
            scene_feat, query.feat = self.blocks[j](
                source=scene_feat, query=query.feat,
                source_pe=scene.pe_list[j], query_pe=query.pe,
                source_batch=scene.batch_list[j], query_batch=query.batch,
                attn_mask=attn_mask if self.with_attn_mask[j] else None
            )
            masks_heatmap, cls_logits, attn_mask = self.mask_module(scene.features[-1], query.feat)
