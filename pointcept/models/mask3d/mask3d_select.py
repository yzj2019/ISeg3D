...
@MODELS.register_module("Mask3dSegmentor-selected")
class Mask3dSegmentor(nn.Module):
    def __init__(self, pcd_backbone=None, mask_decoder=None, loss=None, matcher_cfg=None,
                 mask_threshold=0., num_query=200, topk_per_scene=200, 
                 semantic_ignore=-1, instance_ignore=-1, semantic_background=(0,1)):
        ''' with selected query points
        ...
        self.latent_query = nn.Embedding(num_query, self.mask_decoder.query_embedding_dim)
        # positional encoding
    ...
    def construct_scene(self, pcd_dict):
        ...
        scene.pe_list = [self.get_batched_pe(scene.coord_list[i], scene.batch_list[i]) 
                         for i in range(len(scene.coord_list))]
        return scene
    ...
    def construct_query(self, scene:Scene):
        ...
        query.feat = self.latent_query.weight.repeat(bs, 1).to(scene.feat_list[-1].device)
    ...
    def refine(self, pred_last, in_proj=True):
        ...
        ), device=last_masks_logits.device, dtype=last_masks_logits.dtype)
        pred_last['masks_logits'] = torch.cat([last_masks_logits, one_padding], dim=0).contiguous()

        masks_logits, cls_logits = self.mask_decoder(self.scene, self.query, pred_last, in_proj=in_proj)
        pred_refine = {
