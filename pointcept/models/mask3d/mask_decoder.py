...
class Mask3dDecoderBlock(nn.Module):
    def __init__(
        ...
        self.mlp = FFNLayer(embedding_dim, mlp_dim=mlp_dim, activation=activation, dropout=layer_drop, normalize_before=norm_before)
    ...
    def forward(
        ...
        '''
        query_after_cross = self.masked_query_to_source(source=source, query=query,
...
class Mask3dMaskDecoder(PointModule):
    ...
    def compute_attn_mask(self, masks_logits:Tensor, scene, j, tau=0.5):
        ...
        attn_mask = self.downsample(
            attn_mask.T.contiguous(), scene, 
            from_resolution=self.features_num-1, to_resolution=j
    ...
    def head(self, scene_feat, query_feat):
        ...
        masks_logits = torch.einsum("q d, p d -> q p", mask_feat, scene_feat)   # N_query x N_point
    ...
    def forward(self, scene: Scene, query: Query, pred_last):
        ...
        N_query, N_query_last = query.feat.shape[0], last_masks_logits.shape[0]     # 默认 query 中新的靠后
        assert N_query >= N_query_last, f"assert {N_query} >= {N_query_last}"
        if N_query > N_query_last:
            N_query_new = N_query - N_query_last
            masks_padding, cls_padding = self.head(scene.mask_feat, query.feat[-N_query_new:])
            masks_logits = torch.cat([last_masks_logits, masks_padding], dim=0)  # (N_query, N_point[-1])
            cls_logits = torch.cat([last_cls_logits, cls_padding], dim=0)  # (N_query, num_classes)
        
        pred_init = {
            'masks_logits': masks_logits.contiguous(),
            'cls_logits': cls_logits.contiguous()
        }
        preds = self.transformer_fwd(
            scene, query, pred_init
        )
        return preds
