...
# 这里有大问题：in proj layer是需要学习的，如果是变长输入真的能学好吗？
# -------------------------------------------------------------------------
class SqueezedCrossAttentionLayer(nn.Module):
    ...
    def forward(self, source, query, source_batch, query_batch, attn_mask=None, source_pe=None, query_pe=None):
        ...
        - query: Tensor (n_q, 3, embed_dim)
        - query_batch: (n_q), which batch the query belongs to
        - attn_mask: (n_q, 3, n_s)
        - source_pe: (n_s, embed_dim)
        - query_pe: (n_q, 3, embed_dim)
        - output: (n_q, embed_dim)
        """
        assert query_batch.max() <= source_batch.max(), "query_batch.max() > source_batch.max()"
        B = query_batch.max()+1
        ...
        for i in range(query_batch.shape[0]):
            ...
            if attn_mask != None:
                attn_mask_b = attn_mask[i]       # (3, n_s)
                attn_mask_b = attn_mask_b[:, source_batch_mask] # (3, n_s_b)
            ...
            output[i] = attn_out         # 索引赋值时会正确处理梯度？
        return output
...
class BatchedSelfAttentionLayer(nn.Module):
    ...
    def forward(self, x, pe=None):
        """
        - x: Tensor (n_x, 3, embed_dim)
        - pe: Tensor (n_x, 3, embed_dim)
        - output: Tensor (n_x, 3, embed_dim)
        """
