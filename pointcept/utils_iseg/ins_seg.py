...
def associate_matched_ins(pred, target, void):
    ...
    # 建立索引
    target_new["idx_to_pred_idx"] = {idx_target[i].item():idx_pred[i].item() for i in range(len(idx_pred))}
    ...
    union = pred_new["vert_count"].view(-1,1) + target_new["vert_count"].view(1,-1) # [N_pred, N_target]
