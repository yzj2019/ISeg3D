...
class MyClick:
    def __init__(self, is_positive, coords, index=None, batch_id=None, class_tag=None, feat=None, pe=None):
        '''
...
class Clicks_per_fragment:
    ...
    def add_click(self, click:MyClick, batch_id:int, class_tag:int):
        ...
        按照class增加click
        '''
        self.clicks_idx_map_list[batch_id][class_tag].add_click(len(self.clicks_list), click.is_positive)
        self.clicks_list.append(click)
    ...
    def get_clicks_by_class(self, batch_id:int, class_tag:int):
        ...
        '''
        clicks_idx_list = self.clicks_idx_map_list[batch_id][class_tag].get_clicks_idx_list()
    ...
    def get_latest_clicks(self):
        ...
        for batch_id in range(self.batch_size):
            for class_tag in self.classes[batch_id]:
                init, pos, neg = self.clicks_idx_map_list[batch_id][class_tag].get_latest_pos_neg_idx()
                if init != None:
    ...
    def get_all_cls_ious(self, batch_id:int):
        ...
        return {i : np.array(self.clicks_idx_map_list[batch_id][i].ious, float) 
    ...
    def reset_clicks(self, classes, batch_size):
        self.clicks_list = []
        self.clicks_idx_map_list = [{i:Clicks_idx_per_class() for i in classes[j]} for j in range(batch_size)]



@CLICKER.register_module()
class Clicker3D(object):
    def __init__(self, semantic_ignore_label=-1, instance_ignore_label=-1, max_iou=0.95, sample_num=200):
        '''
    ...
    def set_state(self, mode, fragment_list, semantic=None, instance=None, img_dict=None, fusion=None):
        '''
        ...
        if self.mode == 'test':
            ...
            self.is_testset = len(self.semantic[self.semantic != self.semantic_ignore_label]) == 0      # 判断是不是 test set 做 tester
        else:
        ...
        self.not_ignore_mask = torch.logical_and(self.semantic!=self.semantic_ignore_label, self.instance!=self.instance_ignore_label)
        ...
        for b in range(self.batch_size):
            ...
            self.ins_tag_to_sem_label.append({i:batch_semantic[batch_instance==i][0].item() for i in self.classes[-1]})
        # 为每个fragment维护一个click_list
        self.clicks = [Clicks_per_fragment(self.classes, self.batch_size) for i in range(len(fragment_list))]
    ...
    def generate_fps(self, pcd_dict, fragment_idx=0, fragment_feature=None):
        ...
        sampled_idx = pointops.farthest_point_sampling_with_dist(dist, o, n_o).long().cpu()

        # generate clicks
        end_id = 0
        class_tag = self.instance[pcd_dict["index"]].int().cpu().numpy()        # only for test grid sample that returns "index" attribute
        for i in range(n_o.shape[0]):
            ...
            for j in range(sampled_idx_i.shape[0]):
                if index_i[j] >= pcd_dict["grid_coord"].shape[0]-10:
                    ...
                    continue
                click = MyClick(True, coords_i[j], batch_id=i, class_tag=class_tag_i[j], index=index_i[j])
                self.clicks[fragment_idx].add_click(click, batch_id=i, class_tag=0)         # 不管 class_tag, 一视同仁
    ...
    def make_init_clicks(self, fragment_idx=0, fragment_feature=None):
        ...
        if self.is_testset:
            # 对测试集, 没有真值, 随机采点策略
            return self.generate_fps(fragment, fragment_idx=fragment_idx, fragment_feature=fragment_feature)
        for b in range(self.batch_size):
            select_cls = random.randint(0, len(self.classes[b])-1)
            # 限制 instance 的个数: 训练时除了随机选的一个instance之外, 其余的都以 1-prob 概率被丢弃; 非训练时 prob==1
            if_select = random.choices([False, True], [1-self.choose_prob, self.choose_prob], k=len(self.classes[b]))
            ...
            # for class_tag in self.classes[b]:
                ...
                grid_coord = fragment["grid_coord"][mask.bool()]    # 该scene中对应class_tag的voxel
                ...
                if self.mode != 'train':
                    ...
                    max_distance = max(self.jitter_prob * distance.max(), distance.min())
                    ...
                    index = index[select_idx]
                # index 变成0dim的tensor
                click = MyClick(True, point, batch_id=b, class_tag=class_tag, index=index)
                self.clicks[fragment_idx].add_click(click, batch_id=b, class_tag=class_tag)
    ...
    def make_next_clicks(self, fragment_idx=0):
        ...
        for b in range(self.batch_size):
            ...
            # for class_tag in self.classes[b]:
                # 对每个class
                if self.clicks[fragment_idx].get_cls_ious(b, class_tag)[-1] >= self.max_iou:
                ...
                #     continue
                fp_mask = torch.logical_and(torch.logical_and(torch.logical_not(gt_mask), pred_mask), not_ignore_mask)
                ...
                if self.mode != 'train':
                    ...
                    point = grid_coord[select_idx]
                    index = index[select_idx]
                click = MyClick(is_positive, point, batch_id=b, class_tag=class_tag, index=index)
                self.clicks[fragment_idx].add_click(click, batch_id=b, class_tag=class_tag)
        return self.get_all_latest_clicks(fragment_idx=fragment_idx)
    ...
    def update_ious(self, class_tag_to_iou, batch_id=0):
        ...
        for fragment_idx in range(len(self.clicks)):
            for class_tag in class_tag_to_iou.keys():
                self.clicks[fragment_idx].update_cls_iou(class_tag_to_iou[class_tag].item(), batch_id, class_tag)
